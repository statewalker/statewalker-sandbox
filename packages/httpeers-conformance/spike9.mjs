...
// warm-up
try {
  new AuthorizerBuilder().buildUnauthenticated().authorizeWithLimits(L);
} catch {}
...
const present = (tok, provenPeer, label) => {
  ...
  try {
    b.buildAuthenticated(
      Biscuit.fromBase64(tok.toBase64(), root.getPublicKey()),
    ).authorizeWithLimits(L);
    console.log(`  ${label.padEnd(46)} -> ALLOWED  *** `);
  } catch {
    console.log(`  ${label.padEnd(46)} -> denied`);
  }
};
...
present(
  t.appendBlock(block`connection_peer("THIEF");`),
  "THIEF",
