...
async function fixture(impl: Implementation): Promise<Fixture> {
  ...
  const base: MintInit = {
    mesh: MESH, subject: "user:alice", bound: ALICE, roles: ["app:editor"], expiresAt: SOON,
  };
  return {
    t, mesh, meshPub, device,
    mint: (over = {}) => t.mint({ ...base, ...over }, mesh),
    check: (token, over = {}, rules = RULES) =>
      t.verify(token, meshPub, {
        connectionPeer: ALICE, selfPeer: SERVER, now: NOW, operation: "GET",
        resource: "/notes/2026-08-21", ...over,
      }, rules),
  };
}

const denied = (r: VerifyOutcome, why: string): void => assert(!r.allowed, `${why} — it was ALLOWED`);
...
export const ACCESS_CHECKS: Record<string, Check | { skip: string }> = {
  "A-01": async (impl) => {
    ...
    denied(await f.check(await f.mint(), { operation: "DELETE" }), "nothing grants DELETE, so nothing may permit it");
  },
  ...
  "A-03": { skip: "the forged-header path needs an HTTP surface; established by prototype 03 over real nodes" },
  ...
  "A-06": async (impl) => {
    ...
    assert((bad.failed ?? []).length > 0, "a denial named no failing check (spec P8: a bare boolean is not a decision)");
  },
  ...
  "A-08": async (impl) => {
    ...
    allowed(await f.check(await f.mint({ roles: ["app:editor"] }), { operation: "GET" }),
      "role implication was not transitive");
  },

  "A-09": { skip: "namespace validation is a rule-set build concern; exercised by A-10 once a `policy` capability validates roles" },
  ...
  "A-10": async (impl) => {
    ...
    try {
      impl.policy.build({
        rules: [],
        policies: ['allow if capability("app:never-derived");', 'allow if capability("app:also-never");'],
      });
    } catch (e) {
      msg = e instanceof Error ? e.message : String(e);
    }
    assert(msg, "a policy naming capabilities no rule can derive was accepted");
    assert(msg.includes("app:never-derived") && msg.includes("app:also-never"),
      `the error named only some of the problems: ${msg}`);
  },

  "A-11": async (impl) => {
    const f = await fixture(impl);
    denied(await f.check(await f.mint(), { connectionPeer: THIEF }),
  ...
  "A-12": async (impl) => {
    ...
    denied(await f.check(await f.mint(), { connectionPeer: SERVER, selfPeer: OTHER }),
      "a peer replayed a token it had merely received");
  },

  "A-13": async (impl) => {
    const f = await fixture(impl);
    if (!f.t.forgeDelegation) throw new NotImplemented("no `forgeDelegation` to exercise the un-permitted path");
    ...
    denied(await f.check(attenuated, { connectionPeer: PROXY }),
      "a token whose authority block permitted no delegation was delegated anyway");
  },

  "A-13b": async (impl) => {
    const f = await fixture(impl);
    if (!f.t.forgeDelegation || !f.t.delegate) throw new NotImplemented("needs both `forgeDelegation` and `delegate`");
    ...
    denied(await f.check(forged, { connectionPeer: THIEF }),
      "a PLAIN appended block yielded authority — anyone holding the token can delegate to themselves");
    const signed = await f.t.delegate(delegable, f.meshPub, f.device, PROXY, ['check if operation("GET");']);
    allowed(await f.check(signed, { connectionPeer: PROXY }),
      "a holder-signed third-party delegation was refused");
    denied(await f.check(signed, { connectionPeer: PROXY, operation: "PUT" }),
  ...
  "A-15": async (impl) => {
    ...
    const r = await f.t.verify(await f.mint(), f.t.publicOf(otherMesh), {
      connectionPeer: ALICE, selfPeer: SERVER, now: NOW, operation: "GET", resource: "/notes/x",
    }, RULES);
    denied(r, "a token verified against a different mesh key");
    assert(r.signatureError, "the failure was logical, not cryptographic — the signature was not checked");
  },

  "A-16": { skip: "needs a `directory` capability; no implementation resolves issuer keys through one yet" },
  ...
  "A-18": async (impl) => {
    ...
    denied(await f.check(t, { revokedSubjects: ["user:alice"] }), "a revoked subject was still accepted");
  },

  "A-19": { skip: "asserted by `tests/dependency-graph.test.ts`, which reads manifests rather than calling the implementation" },

  "A-20": async (impl) => {
    const f = await fixture(impl);
    const future = await f.mint({ extraChecks: ["check if quantum_safe(true);"] });
    denied(await f.check(future),
      "a constraint the verifier has never heard of was IGNORED rather than refused");
    allowed(await f.check(future, { extraFacts: ["quantum_safe(true);"] }),
  ...
  "A-21": async (impl) => {
    ...
    denied(await f.check(phone, { connectionPeer: PHONE, revokedBindings: [PHONE] }),
      "revoking a binding did not withdraw that device");
    allowed(await f.check(laptop, { revokedBindings: [PHONE] }),
  ...
  "A-23": async (impl) => {
    ...
    const widened = await f.t.attenuate(await f.mint({ roles: ["app:reader"] }), f.meshPub, ['role("app:editor");']);
    denied(await f.check(widened, { operation: "PUT" }),
    ...
    allowed(await f.check(await f.mint({ roles: ["app:editor"] }), { operation: "PUT" }),
  ...
  "A-24": async (impl) => {
    ...
    allowed(await f.check(scoped, { selfPeer: SERVER }), "an audience-scoped token failed at its intended destination");
    denied(await f.check(scoped, { selfPeer: OTHER }),
