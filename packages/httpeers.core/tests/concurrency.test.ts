...
describe("T-3: the outbound semaphore", () => {
  ...
  it("load past the width degrades gracefully: every call completes late rather than failing, and the peak concurrent stream count never exceeds the configured width", async () => {
  ...
  it("a permit is released after a failed call -- one bad target never permanently strands a slot for calls after it", async () => {
    const server = await node(true);
    const mounts = createMounts();
  ...
  it("row 3a: this client's own outbound cap trips (rejects without queueing) and surfaces as PeerStreamResetError", async () => {
  ...
  it("the at-limit contract: calls that cannot get a permit within requestTimeoutMs reject with PeerRequestTimeoutError -- bounded, never an unbounded queue, never a silent reset", async () => {
