...
const q = await fetchOverDuplex(call, new Request("http://peer/echo-query?group=alpha&limit=7"));
...
console.log(
  `  server saw: ${JSON.stringify(qBody.query)}  ${queryOk ? "\x1b[32m✓\x1b[0m" : "\x1b[31m✗ dropped\x1b[0m"}`,
);
...
console.log(
  `  server received: ${JSON.stringify(joined)} in ${bBody.count} chunk(s)  ${bodyOk ? "\x1b[32m✓\x1b[0m" : "\x1b[31m✗ empty\x1b[0m"}`,
);
